...
impl ArrayGenerator for RandomBooleanGenerator {
    fn generate(
        ...
        let bytes = BooleanBuffer::new(Buffer::from_vec(bytes), 0, length.0 as usize);
...
impl<T: ArrowPrimitiveType + Send + Sync> ArrayGenerator for RandomBytesGenerator<T> {
    fn generate(
        ...
        let mut bytes = vec![0; num_bytes as usize];
        rng.fill_bytes(&mut bytes);
        let bytes = ScalarBuffer::new(Buffer::from_vec(bytes), 0, length.0 as usize);
        Ok(Arc::new(
...
impl<T: ByteArrayType> ArrayGenerator for FixedBinaryGenerator<T> {
    fn generate(
        ...
        let bytes = Buffer::from_vec(Vec::from_iter(
            self.value
                .iter()
                .cycle()
        ...
        let offsets =
            OffsetBuffer::from_lengths(std::iter::repeat_n(self.value.len(), length.0 as usize));
        Ok(Arc::new(arrow_array::GenericByteArray::<T>::new(
            offsets, bytes, None,
        )))
    }
